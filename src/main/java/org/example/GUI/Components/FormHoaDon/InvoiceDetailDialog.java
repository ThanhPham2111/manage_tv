...
public class InvoiceDetailDialog extends JDialog{
    ...
    private void initBottomPanel(){
        ...
        // bottomPanel.add(createRow(null, printInvoiceBtn, FlowLayout.TRAILING), BorderLayout.SOUTH);
    }

    private void loadInvoice(){
        invoiceDetailsTableModel.setRowCount(0);
        InvoiceDTO invoice = invoiceBUS.getInvoiceByID(maHoaDon);
        ArrayList<InvoiceDetailDTO> invoiceDetails = invoiceBUS.getInvoiceDetailsByID(maHoaDon);
        ...
        for(InvoiceDetailDTO invoiceDetail: invoiceDetails){
            invoiceDetailsTableModel.addRow(new Object[]{
                invoiceDetail.getMaSP(),
                invoiceDetail.getTenSP(),
    ...
    private void styleTable(){
        ...
        UtilsJTableStyle.setStyleTableCell(invoiceDetailsTable, FONT_LABEL);

        UtilsJTableStyle.setAlignmentTableCell(invoiceDetailsTable, 0, SwingConstants.LEFT);
        UtilsJTableStyle.setAlignmentTableCell(invoiceDetailsTable, 1, SwingConstants.LEFT);
        UtilsJTableStyle.setAlignmentTableCell(invoiceDetailsTable, 2, SwingConstants.CENTER);
        UtilsJTableStyle.setAlignmentTableCell(invoiceDetailsTable, 3, SwingConstants.RIGHT);
        UtilsJTableStyle.setAlignmentTableCell(invoiceDetailsTable, 4, SwingConstants.RIGHT);
        
        
        UtilsJTableStyle.setWidthTableColumns(invoiceDetailsTable, sp.getPreferredSize().width,
                                            18, 27, 8, 17, 20);
    }
