...
public class SupplierDisable extends JPanel {
    ...
    public SupplierDisable() {
        ...
        if (addIcon != null) btnAdd.setIcon(addIcon);
        ...
        if (viewIcon != null) btnView.setIcon(viewIcon);
        ...
        if (exportIcon != null) btnExportExcel.setIcon(exportIcon);
        ...
        if (importIcon != null) btnImportExcel.setIcon(importIcon);
        ...
        if (refreshIcon != null) btnRefresh.setIcon(refreshIcon);
    ...
    private void showDialogToAddSupplier() {
        addDialog = new JDialog(SwingUtilities.getWindowAncestor(this), "Thêm nhà cung cấp", Dialog.ModalityType.APPLICATION_MODAL);
    ...
    private void editSelectedSupplier() {
        ...
        if (selectedRow != -1) {
            ...
            if (supplier != null) {
                editDialog = new JDialog(SwingUtilities.getWindowAncestor(this), "Sửa nhà cung cấp", Dialog.ModalityType.APPLICATION_MODAL);
    ...
    private void viewSelectedSupplier() {
        ...
        if (selectedRow != -1) {
            ...
            SupplierDTO supplier = supplierBus.getSupplierDTO(maNCC);
            if (supplier != null) {
                JDialog viewDialog = new JDialog(SwingUtilities.getWindowAncestor(this), "Xem nhà cung cấp", Dialog.ModalityType.APPLICATION_MODAL);
            ...
            JOptionPane.showMessageDialog(this, "Vui lòng chọn một nhà cung cấp để xem", "Thông báo", JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    private JPanel createSupplierFormPanel(SupplierDTO supplier) {
        ...
        if (supplier != null) cbTrangThai.setSelectedItem(convertTrangThai(supplier.getTrangThai()));
        ...
        btnOK.addActionListener(e -> {
            ...
            if (checkInfo(supplier == null ? addDialog : editDialog, txtMaNCC.getText(), txtSDT.getText())) {
                SupplierDTO newSupplier = new SupplierDTO(txtMaNCC.getText(), txtTenNCC.getText(), txtDiaChi.getText(), txtSDT.getText(), txtFax.getText(), trangThai);
                if (supplier == null) {
                    supplierBus.add(newSupplier);
                    JOptionPane.showMessageDialog(addDialog, "Thêm nhà cung cấp thành công!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
                    addDialog.dispose();
                } else {
                    supplierBus.update(newSupplier);
                    JOptionPane.showMessageDialog(editDialog, "Sửa nhà cung cấp thành công!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
                    editDialog.dispose();
                }
    ...
    private void importFromExcel() {
        ...
        if (fileChooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            try (FileInputStream inputStream = new FileInputStream(fileToOpen); Workbook workbook = new XSSFWorkbook(inputStream)) {
    ...
    private void refreshTable() {
        SwingUtilities.invokeLater(() -> {
            ...
            buttonPanel.setVisible(option.equals("Đang hoạt động"));
            for (SupplierDTO sup : supplierList) {
                if (sup.getTrangThai() == trangThai) {
                    model.addRow(new Object[]{sup.getMaNCC(), sup.getTenNCC(), sup.getDiaChi(), sup.getSDT(), sup.getFax(), convertTrangThai(sup.getTrangThai())});
                }
            }
    ...
    private void addFocusListenerToTextField(String defaultText) {
        textFieldSearch.addFocusListener(new FocusAdapter() {
            ...
            public void focusGained(FocusEvent e) {
                if (textFieldSearch.getText().equals(defaultText)) textFieldSearch.setText("");
            }

            @Override
            public void focusLost(FocusEvent e) {
                if (textFieldSearch.getText().isEmpty()) textFieldSearch.setText(defaultText);
            }
        });
    }

    private void showConfirmationDialog(String maNCC) {
        int option = JOptionPane.showConfirmDialog(this, "Bạn có muốn khôi phục nhà cung cấp này?", "Xác nhận", JOptionPane.YES_NO_OPTION);
        if (option == JOptionPane.YES_OPTION) {
            supplierBus.updateTrangthai(maNCC, 0);
            JOptionPane.showMessageDialog(this, "Khôi phục nhà cung cấp thành công!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
            SwingUtilities.invokeLater(this::refreshTable); // Cập nhật bảng ngay lập tức trong EDT
        }
    }
